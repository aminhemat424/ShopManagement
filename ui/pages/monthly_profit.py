...
from core.database import get_db_connection, calculate_monthly_profit, calculate_total_expenses
from datetime import datetime
class MonthlyProfitPage(QWidget):
    def __init__(self):
        super().__init__()
        self._init_ui()
    ...
    def _init_ui(self):
        ...
        title = QLabel("Monthly Sales & Profit Report")
        ...
        # Monthly breakdown
        breakdown_title = QLabel("This Month's Sales Breakdown")
        ...
        self.table.setHorizontalHeaderLabels(["Date", "Sale Type", "Amount", "Status"])
    ...
    def _load_data(self):
        try:
            # Get profit data
            profit_data = calculate_monthly_profit()
            
            # Get this month's expenses
            month_start = datetime.now().replace(day=1).strftime("%Y-%m-%d")
            month_end = datetime.now().strftime("%Y-%m-%d")
            month_expenses = calculate_total_expenses(month_start, month_end)
            ...
            for card in [self.total_sales_card, self.total_profit_card, self.expenses_card, self.net_profit_card, self.profit_margin_card]:
                ...
                if value_type == "total_sales":
                    ...
                    card.value_label.setText(f"${month_expenses:.2f}")
